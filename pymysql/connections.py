...
class Connection:
    ...
    def close(self):
        ...
        """
        print('PYMYSQL: Entering close')
        if self._closed:
            print('PYMYSQL: socket is already closed')
            raise err.Error("Already closed")
        self._closed = True
        if self._sock is None:
            print('PYMYSQL: socket is None')
            return
        send_data = struct.pack("<iB", 1, COMMAND.COM_QUIT)
        try:
            print('PYMSQL: sending the close command over socket to mysql')
            self._write_bytes(send_data)
        except Exception as err:
            print(f'PYMYSQL: Exception in close: {err}')
            pass
        finally:
            print('PYMYSQL: calling force close')
            self._force_close()

    @property
    def open(self):
        """Return True if the connection is open"""
        print(f'PYMYSQL: opening the connection: {self._sock}')
        return self._sock is not None

    def _force_close(self):
        """Close connection without QUIT message"""
        print('PYMYSQL: inside _force_close')
        if self._sock:
            print('PYMYSQL: socket is alive, calling _sock.close()')
            try:
                self._sock.close()
            except:  # noqa
                print('PYMYSQL: there was an exception trying to force close the socket')
                pass
        self._sock = None
        self._rfile = None
    ...
    def connect(self, sock=None):
        ...
        try:
            ...
            print(f'PYMYSQL: exception on connect {e}')
            self._rfile = None
            if sock is not None:
                print('PYMYSQL: connect socket is not None going to try and close')
                try:
                    sock.close()
                except:  # noqa
                    print('PYMYSQL: there was an exception in connect on close')
                    pass
    ...
    def _read_bytes(self, num_bytes):
        ...
        while True:
            try:
                ...
                self._force_close()
                raise err.OperationalError(
                    CR.CR_SERVER_LOST, "Lost connection to MySQL server during query (%s)" % (e,),
                )
        ...
        if plugin_name == b"caching_sha2_password":
            ...
            while True:
                ...
                if prompt == b"Password: ":
                    ...
                    raise err.OperationalError(
                        2059, "Authentication plugin '%s' (%r) not configured" % (plugin_name, handler),
                    )
                pkt = self._read_packet()
                pkt.check_error()
